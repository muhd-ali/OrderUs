...
@IBDesignable public class GMStepper: UIControl {
    ...
    */
    @objc let labelSlideLength: CGFloat = 5

    /// Duration of the sliding animation
    @objc let labelSlideDuration = TimeInterval(0.1)

    /// Duration of the animation when the value hits the limit.
    @objc let limitHitAnimationDuration = TimeInterval(0.1)
    ...
    /** When UIStepper reaches its top speed, it alters the value with a time interval of ~0.05 sec.
        ...
        - Then, every 0.05 sec.
    */
    @objc let timerInterval = TimeInterval(0.05)
    ...
    @objc var timerFireCount = 0
    @objc var timerFireCountModulo: Int {
...
extension GMStepper {
    ...
    @objc func animateSlideLeft() {
        UIView.animate(withDuration: labelSlideDuration) {
            self.label.center.x -= self.labelSlideLength
        }
    }

    @objc func animateSlideRight() {
        UIView.animate(withDuration: labelSlideDuration) {
            self.label.center.x += self.labelSlideLength
        }
    }

    @objc func animateToOriginalPosition() {
    ...
    @objc func animateLimitHitIfNeeded() {
        if value == minimumValue {
    ...
    @objc func animateLimitHitForButton(button: UIButton){
